...
import {
  getAbbreviation,
  getFirstWordAbbreviation,
  getFirstLetterAbbreviation,
  splitToChars,
} from "@/utils/textUtils";
import { getSkillTextVariants } from "@/components/responsive/SkillText";
import {
  HomeTitleVariants,
  HomeSubtitleVariants,
  HomeDescriptionVariants,
} from "@/components/responsive/HomeContent";
import {
  LandingTitleVariants,
  LandingSubtitleVariants,
} from "@/components/responsive/LandingContent";
  ...
  return (
    <div className={`relative group ${className}`}>
      ...
      <div className="relative bg-black rounded-lg px-2 py-4 sm:px-3 sm:py-5 md:px-4 md:py-4 flex items-center gap-1.5 sm:gap-2 md:gap-3 border border-gray-800 transition-transform w-full min-h-[70px] sm:min-h-[80px] md:min-h-[90px] md:h-[90px]">
        ...
        >
          {Object.entries(textVariants).map(([key, content]) => (
            <span key={key} className={`skill-${key}`}>
              {content}
            </span>
          ))}
        </span>
...
const CloseButton = ({ onClick, minimized = false }) => (
  <Button
    onClick={onClick}
    className={`action-btn action-btn-close bg-gray-900 text-white hover:bg-gray-800 border border-gray-700 hover:border-red-500 w-10 h-10 md:w-auto md:h-auto md:px-2 md:py-1.5 p-0 rounded-lg animate-pulse-slow transition-all duration-300 group ${
      minimized ? "action-btn-minimized" : ""
    }`}
  >
    <X className="action-btn-icon w-6 h-6 md:w-4 md:h-4 md:mr-2.5 group-hover:rotate-90 transition-transform duration-300" />
    <span className="hidden md:inline transition-opacity duration-300">
      Close
    </span>
  </Button>
);

const BackToHomeButton = ({ onClick, minimized = false }) => (
  <Button
    onClick={onClick}
    className={`action-btn action-btn-back-to-home bg-gray-900 hover:bg-gray-800 border border-gray-700 hover:border-purple-500 text-purple-500 w-10 h-10 md:w-auto md:h-auto md:px-3 md:py-1.5 p-0 rounded-lg animate-pulse-slow transition-all duration-300 group ${
      minimized ? "action-btn-minimized" : ""
    }`}
  >
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-8 bg-black home-particles">
      <div className="text-center space-y-6 max-w-3xl home-content">
        <h1 className="text-5xl md:text-7xl font-bold text-white animate-pulse section-title home-title">
          {Object.entries(HomeTitleVariants()).map(([key, content]) => (
            <span key={key} className={`home-title-${key}`}>
              {content}
            </span>
          ))}
        </h1>
        <h2 className="text-2xl md:text-4xl font-semibold text-white section-subtitle home-subtitle">
          {Object.entries(HomeSubtitleVariants()).map(([key, content]) => (
            <span key={key} className={`home-subtitle-${key}`}>
              {content}
            </span>
          ))}
        </h2>
        <p className="text-lg md:text-xl text-gray-300 leading-relaxed section-description home-description">
          {Object.entries(HomeDescriptionVariants()).map(([key, content]) => (
            <span key={key} className={`home-description-${key}`}>
              {content}
            </span>
          ))}
        </p>
        ...
        <div className="min-h-screen p-8">
          <div className="text-center mb-12 pt-8">
            <h1 className="text-[2.16rem] md:text-[3.375rem] font-semibold mb-4 font-jetbrains-mono flex flex-col md:flex-row md:justify-center md:items-center md:gap-2 landing-page-title">
              {Object.entries(LandingTitleVariants()).map(([key, content]) => (
                <span key={key} className={`landing-title-${key}`}>
                  {content}
                </span>
              ))}
            </h1>
            <p className="text-xl text-gray-300 landing-subtitle">
              {Object.entries(LandingSubtitleVariants()).map(
                ([key, content]) => (
                  <span key={key} className={`landing-subtitle-${key}`}>
                    {content}
                  </span>
                )
              )}
